...
import KeyCloakCookieStore from '../libs/keyCloakCookieStore.js'
...
export default class KeyCloakService {
    ...
    static initKeyCloak(config) {
        ...
        result.config.enableBasicAuth = false;
        ...
        result.accessDenied = (request, response) => response.json({ message: "Access Deny." });
        return result;
    }
    ...
    loginUser(login, password, remember, request, response) {
        return this.keyCloak.grantManager.obtainDirectly(login, password, remember).then(grant => {
    ...
    checkPermission(request, resource, scope) {
        if (!request || !request.headers || !request.headers.authorization) {
            return Promise.reject('Authorization header not found');
        }
        console.log("resource:", resource, " - scope:", scope);
        let permission = {
            id: resource,
            scopes: [scope]
        };
        ...
        console.log(permission, " - ", authzRequest);
        //Thoilc(*Note)-Get token authorization
