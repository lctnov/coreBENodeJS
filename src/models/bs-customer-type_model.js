...
export default class CustomerTypeModel {
    ...
    async loadCustomerType(req) {
        ...
        var query = await this.cfsglobal
            .from("bs_customer_type")
            .select('id', 'customer_type_code', 'customer_type_name')
            ...
            switch (item.status) {
                case 'insert':
                    if (!item.customer_type_code) {
                        response['iStatus'] = false;
                        response['iMessage'] = 'Vui lòng nhập mã loại khách hàng!';
                        return response;
                    }

                    if (!item.customer_type_name) {
                        response['iStatus'] = false;
                        response['iMessage'] = 'Vui lòng nhập tên loại khách hàng!';
                        return response;
                    }
                    ...
                    let checkCode = await this.cfsglobal
                        .from("bs_customer_type")
                        .select("rowguid")
                        .where("customer_type_code", item.customer_type_code)
                        .catch(err => console.log(err)) || [];

                    if (checkCode && checkCode.length) {
                        response['iStatus'] = false;
                        response['iMessage'] = "Mã loại khách hàng đã tồn tại!";
                        return response;
                    }
                    // -----------------------------------------------
                    let obj = {
                    ...
                    try {
                        return await this.cfsglobal.from("bs_customer_type")
                            .insert(obj)
                            .returning('*')
                            .then(data => {
                                response['iStatus'] = true;
                ...
                case 'update':
                    ...
                    try {
                        ...
                        return await this.cfsglobal.from("bs_customer_type").where('id', item.id)
                            .update({
                                customer_type_code: item.customer_type_code || null,
                                customer_type_name: item.customer_type_name || null,
